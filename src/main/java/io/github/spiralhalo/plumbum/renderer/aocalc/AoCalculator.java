...
public class AoCalculator {
	...
	public void compute(QuadEmitterImpl quad, boolean isVanilla) {
		...
		case EMULATE:
			calcFastVanilla(quad);
			shouldCompare = Plumbum.DEBUG_COMPARE_LIGHTING && isVanilla;
			break;

		case HYBRID:
		default:
			if (isVanilla) {
				shouldCompare = Plumbum.DEBUG_COMPARE_LIGHTING;
				calcFastVanilla(quad);
			} else {
				shouldCompare = false;
				calcEnhanced(quad);
	...
	private AoFaceData computeFace(Direction lightFace, boolean isOnBlockFace, boolean shade) {
		...
		if ((completionFlags & mask) == 0) {
			...
			if (!Plumbum.FIX_SMOOTH_LIGHTING_OFFSET) searchPos.move(lightFace);
	...
	private static int meanBrightness(int a, int b, int c, int d) {
		if (Plumbum.FIX_SMOOTH_LIGHTING_OFFSET) {

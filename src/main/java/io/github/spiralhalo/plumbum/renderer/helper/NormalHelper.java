...
public abstract class NormalHelper {
	...
	/**
	 * Version of {@link #packNormal(float, float, float, float)} that accepts a Frex packed vector
	 */
	public static int packNormal(int packedNormal, float w) {
		return packNormal(PackedVector3f.unpackX(packedNormal), PackedVector3f.unpackY(packedNormal), PackedVector3f.unpackZ(packedNormal), w);
	}
	 ...
	 */
	public static int computeFaceNormal(QuadView q) {
		final Direction nominalFace = q.nominalFace();

		if (GeometryHelper.isQuadParallelToFace(nominalFace, q)) {
			Vec3i vec = nominalFace.getVector();
			return PackedVector3f.pack(vec.getX(), vec.getY(), vec.getZ());
		}
		...
		return PackedVector3f.pack(normX, normY, normZ);
	}
}

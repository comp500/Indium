...
public abstract class EncodingFormat {
	...
	static {
		...
		Preconditions.checkState(VERTEX_STRIDE == QuadView.VANILLA_VERTEX_STRIDE, "Plumbum vertex stride (%s) mismatched with rendering API (%s)", VERTEX_STRIDE, QuadView.VANILLA_VERTEX_STRIDE);
		Preconditions.checkState(QUAD_STRIDE == QuadView.VANILLA_QUAD_STRIDE, "Plumbum quad stride (%s) mismatched with rendering API (%s)", QUAD_STRIDE, QuadView.VANILLA_QUAD_STRIDE);
	}

	/** used for quick clearing of quad buffers. */
	static final int[] EMPTY = new int[TOTAL_STRIDE];

	private static final int DIRECTION_MASK = MathHelper.smallestEncompassingPowerOfTwo(FaceUtil.UNASSIGNED_INDEX) - 1;
	...
	static {
		Preconditions.checkArgument(MATERIAL_SHIFT + MATERIAL_BIT_COUNT <= 32, "Plumbum header encoding bit count (%s) exceeds integer bit length)", TOTAL_STRIDE);
	}

	static Direction cullFace(int bits) {
		return FaceUtil.faceFromIndex((bits >> CULL_SHIFT) & DIRECTION_MASK);
	}

	static int cullFace(int bits, Direction face) {
		return (bits & CULL_INVERSE_MASK) | (FaceUtil.toFaceIndex(face) << CULL_SHIFT);
	}

	static Direction lightFace(int bits) {
		return FaceUtil.faceFromIndex((bits >> LIGHT_SHIFT) & DIRECTION_MASK);
	}

	static int lightFace(int bits, Direction face) {
		return (bits & LIGHT_INVERSE_MASK) | (FaceUtil.toFaceIndex(face) << LIGHT_SHIFT);
	}

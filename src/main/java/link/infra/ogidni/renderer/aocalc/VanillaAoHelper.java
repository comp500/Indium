...
import link.infra.ogidni.renderer.accessor.AccessBlockModelRenderer;
...
public class VanillaAoHelper {
	private static Supplier<link.infra.ogidni.renderer.accessor.AccessAmbientOcclusionCalculator> factory;
	...
	private static link.infra.ogidni.renderer.accessor.AccessBlockModelRenderer blockRenderer;
	...
	public static void initialize(BlockModelRenderer instance) {
		...
		for (Class<?> innerClass : BlockModelRenderer.class.getDeclaredClasses()) {
			if (innerClass.getName().equals(target)) {
				...
				factory = new Supplier<link.infra.ogidni.renderer.accessor.AccessAmbientOcclusionCalculator>() {
					@Override
					public link.infra.ogidni.renderer.accessor.AccessAmbientOcclusionCalculator get() {
						try {
							return (link.infra.ogidni.renderer.accessor.AccessAmbientOcclusionCalculator) constructor.newInstance(instance);
						} catch (Exception e) {

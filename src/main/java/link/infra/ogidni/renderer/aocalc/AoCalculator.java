...
import link.infra.ogidni.renderer.accessor.AccessAmbientOcclusionCalculator;
import link.infra.ogidni.renderer.helper.GeometryHelper;
import link.infra.ogidni.renderer.mesh.EncodingFormat;
import link.infra.ogidni.renderer.render.BlockRenderInfo;
...
import link.infra.ogidni.Indigo;
import link.infra.ogidni.renderer.mesh.MutableQuadViewImpl;
...
public class AoCalculator {
	...
	private final link.infra.ogidni.renderer.render.BlockRenderInfo blockInfo;
	...
	private void calcFastVanilla(MutableQuadViewImpl quad) {
		...
		if ((flags & GeometryHelper.LIGHT_FACE_FLAG) == 0 && (flags & GeometryHelper.AXIS_ALIGNED_FLAG) == GeometryHelper.AXIS_ALIGNED_FLAG && Block.isShapeFullCube(blockInfo.blockState.getCollisionShape(blockInfo.blockView, blockInfo.blockPos))) {
			flags |= GeometryHelper.LIGHT_FACE_FLAG;
		}

		if ((flags & GeometryHelper.CUBIC_FLAG) == 0) {
			vanillaPartialFace(quad, (flags & GeometryHelper.LIGHT_FACE_FLAG) != 0);
		} else {
			vanillaFullFace(quad, (flags & GeometryHelper.LIGHT_FACE_FLAG) != 0);
		}
	}

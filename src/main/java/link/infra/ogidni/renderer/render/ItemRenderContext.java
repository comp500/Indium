...
import link.infra.ogidni.renderer.helper.ColorHelper;
import link.infra.ogidni.renderer.mesh.EncodingFormat;
import link.infra.ogidni.renderer.mesh.MeshImpl;
 ...
 */
public class ItemRenderContext extends link.infra.ogidni.renderer.render.AbstractRenderContext implements RenderContext {
	...
	private void renderQuad() {
		...
		final int lightmap = mat.emissive(0) ? link.infra.ogidni.renderer.render.AbstractQuadRenderer.FULL_BRIGHTNESS : this.lightmap;
	...
	private void renderFallbackWithTransform(List<BakedQuad> quads, Direction cullFace) {
		...
		for (final BakedQuad q : quads) {
			editorQuad.fromVanilla(q, OgidniRenderer.MATERIAL_STANDARD, cullFace);
			renderQuad();
		}
	}
